...
sub init {
    ...
    # Hook on the can_join event to forward users if needed.
    $pool->on('user.can_join' => \&on_user_can_join, after=> 'in.channel', name => 'check.forward');
    $pool->on('user.join_failed' => \&on_user_join_failed, with_eo => 1, name => 'join.failed');
    return 1;
}
...
sub cmode_forward {
    ...
    # setting.
        ...
        $source->numeric(ERR_NOSUCHCHANNEL => $mode->{param}) and return if (!$f_channel 
            && $source->isa('user'));
